 ...
 *
 */
var StandardRecommendEdit = {
    id: "standardRecommendEditTable",//表格id
    seItem: null,		//选中的条目
    table: null,
    layerIndex: -1,
    PARENT_ID:-1,
...
StandardRecommendEdit.showFoodDemo = function(param){
    $("#foodDemo").empty();
    var queryData = {};
    ...
    var ajax = new $ax(Feng.ctxPath + "/food/foodList", function (data) {
        ...
        for(var i=0;i<foodList.length;i++){
            ...
            $("#foodDemo").append("<li value='"+id+"' onclick='StandardRecommendEdit.clickFoodDemo(\""+param+"\",\""+name+"\",\""+id+"\")' >"+name+"</li>")
        }
...
StandardRecommendEdit.clickFoodDemo = function (param,name,id) {
    $("#"+param).val(name);
...
function onBodyDown(event) {
    if (!(event.target.id == "menuBtn" || event.target.id == "menuContent" || $(
        ...
        StandardRecommendEdit.hideDeptSelectTree();
    }
}
 ...
 * 隐藏部门选择的树
 */
StandardRecommendEdit.hideDeptSelectTree = function () {
 ...
 * 提交添加用户
 */
StandardRecommendEdit.editSubmit = function () {
    var queryData = {};
 ...
 * 关闭此对话框
 */
StandardRecommendEdit.close = function () {
